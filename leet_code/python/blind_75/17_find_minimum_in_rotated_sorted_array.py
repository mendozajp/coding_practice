...
class Solution:
    def findMin(self, nums: List[int]) -> int:
        ...
        while l < r:                            # simple enough, I tried this and with the way I was setting l and r led to me thinking this wasn't it
            m = l + (r - l) // 2                # I didnt add l, I just had (r-l) // 2, but I kept adding it to a saved index
            if nums[m] < nums[r]:               # Saw something different, I saw, if right most is smaller then current, min should be between current and right
                r = m                           # similar enough, this is if current is smaller then right, everything to the right is increasing, ignore
            else:                               
                l = m + 1                       # m + 1 to keep things moving maybe? I started with r and l = idx, but kept hitting endless loops
